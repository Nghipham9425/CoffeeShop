      ...
      {showForm ? (
        ...
        >
          <form onSubmit={handleSubmit} className="grid gap-4 p-5 md:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-bold text-[#553B2F]">
              Slug
              <input className="h-11 rounded-lg border border-[#C7A792] px-3 outline-none focus:border-[#553B2F]" value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: event.target.value }))} placeholder="Bỏ trống để tự tạo" />
            </label>
            <label className="grid gap-2 text-sm font-bold text-[#553B2F]">
              Đơn vị
