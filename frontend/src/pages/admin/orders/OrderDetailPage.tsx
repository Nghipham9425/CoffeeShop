import { ArrowLeft, ArrowRight, CheckCircle2, CircleX, CreditCard, PackageCheck, Truck } from "lucide-react";
...
export function OrderDetailPage() {
  ...
  async function updateShipment(status: NonNullable<AdminOrder["shipment"]>["status"]) {
    if (!token || !order) return;
    const needsDeliveryInfo = status === "PACKED" && !order.shipment?.trackingCode;
    const carrier = needsDeliveryInfo ? window.prompt("Đơn vị vận chuyển:", order.shipment?.carrier ?? "GHN")?.trim() : order.shipment?.carrier ?? undefined;
    const trackingCode = needsDeliveryInfo ? window.prompt("Mã vận đơn:", "")?.trim().toUpperCase() : order.shipment?.trackingCode ?? undefined;
    if (needsDeliveryInfo && (!carrier || !trackingCode)) { setError("Cần nhập đơn vị vận chuyển và mã vận đơn trước khi đóng gói."); return; }
    try {
      setSaving(true); setError("");
      await adminApi.upsertShipment(token, order.id, { status, carrier, trackingCode });
      await loadOrder();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Không thể cập nhật giao nhận."); }
    finally { setSaving(false); }
  }
  ...
  const orderAction: Partial<Record<AdminOrder["status"], { status: AdminOrder["status"]; label: string }>> = {
    ...
    PACKING: { status: "SHIPPING", label: "Chuyển sang đang giao" },
    SHIPPING: { status: "COMPLETED", label: "Hoàn tất đơn hàng" },
  };
  const shipmentStatus = order.shipment?.status ?? "WAITING";
  const shipmentAction: Partial<Record<typeof shipmentStatus, { status: typeof shipmentStatus; label: string }>> = {
    WAITING: { status: "PACKED", label: "Đóng gói và cấp vận đơn" },
  ...
  return <AdminPageShell title={`Đơn hàng ${order.orderCode}`} description="Theo dõi khách hàng, dòng hàng, thanh toán và giao nhận trên một màn hình.">
    <div className="mb-5"><Button asChild variant="outline" className="border-[#C7A792] text-[#553B2F] hover:bg-[#f8f2ed]"><Link to="/admin/don-hang"><ArrowLeft size={16} />Quay lại danh sách</Link></Button></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[1.55fr_1fr]">
      ...
      <div className="space-y-5">
        <AdminPanel title="Trạng thái xử lý"><div className="space-y-4 p-5"><div className="flex flex-wrap items-center gap-3"><AdminStatusBadge status={order.status} /><span className="text-sm font-semibold text-[#7A665D]">Cập nhật: {formatDate(order.updatedAt)}</span></div>{orderAction[order.status] ? <Button disabled={saving} onClick={() => void updateOrderStatus(orderAction[order.status]!.status)} className="w-full bg-[#553B2F] text-white hover:bg-[#3c271f]">{orderAction[order.status]!.label}<ArrowRight size={16} /></Button> : null}{["PENDING", "CONFIRMED"].includes(order.status) ? <Button disabled={saving} variant="outline" onClick={() => void updateOrderStatus("CANCELLED")} className="w-full border-red-200 text-red-700 hover:bg-red-50"><CircleX size={16} />Hủy đơn</Button> : null}</div></AdminPanel>
        ...
        <AdminPanel title="Giao nhận"><div className="space-y-3 p-5 text-sm"><p className="flex items-center gap-2 font-black text-[#553B2F]"><Truck size={17} />{order.shipment?.carrier ?? "Nội bộ / chưa bàn giao"}</p><Info label="Mã vận đơn" value={order.shipment?.trackingCode ?? "Chưa cấp"} /><Info label="Trạng thái giao hàng" value={shipmentStatus} />{shipmentAction[shipmentStatus] ? <Button disabled={saving} onClick={() => void updateShipment(shipmentAction[shipmentStatus]!.status)} variant="outline" className="w-full border-[#C7A792] text-[#553B2F] hover:bg-[#f8f2ed]"><PackageCheck size={16} />{shipmentAction[shipmentStatus]!.label}</Button> : null}{shipmentStatus === "DELIVERED" ? <p className="rounded-md bg-emerald-50 p-3 text-xs font-bold text-emerald-800">Đã giao thành công, có thể hoàn tất đơn hàng.</p> : null}</div></AdminPanel>
      </div>
    </div>
  </AdminPageShell>;
}
