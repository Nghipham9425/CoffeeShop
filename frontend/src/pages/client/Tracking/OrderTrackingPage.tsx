...
export function OrderTrackingPage() {
  ...
  return <main className="min-h-[65vh] bg-stone-50 px-4 py-14"><section className="mx-auto max-w-3xl">
    <div className="text-center"><Badge>Hỗ trợ khách hàng</Badge><h1 className="mt-4 font-serif text-4xl font-black text-stone-950 md:text-5xl">Tra cứu đơn hàng</h1><p className="mx-auto mt-4 max-w-xl text-stone-600">Nhập mã đơn hàng hoặc mã vận đơn để kiểm tra tiến độ xử lý và giao nhận.</p></div>
    <Card className="mt-8"><CardContent className="p-6"><form onSubmit={submit} className="grid gap-4 md:grid-cols-[1fr_auto]">
      <input value={trackingCode} onChange={(event) => setTrackingCode(event.target.value.toUpperCase())} className="h-12 rounded-xl border border-stone-200 px-4 font-semibold outline-none focus:border-[var(--coffee)]" placeholder="Ví dụ: PT1234567890 hoặc GHN123456789" required />
    ...
    {!order && !loading && !error ? <div className="mt-8 flex items-center justify-center gap-2 text-sm font-semibold text-stone-500"><PackageSearch size={18} /> Mã vận đơn được cập nhật khi đơn hàng được bàn giao vận chuyển; trước đó bạn vẫn có thể tra bằng mã đơn hàng.</div> : null}
  </section></main>;
}
