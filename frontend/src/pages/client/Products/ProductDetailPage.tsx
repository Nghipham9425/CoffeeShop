...
const suggestionTitles = [
  "Cà phê rang mộc cho quán",
  "Blend espresso ổn định",
  "Gói OEM nhãn riêng",
];
...
export function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([publicApi.product(productId), publicApi.products()])
      ...
      .catch((err) => {
        if (alive) setError(err instanceof Error ? err.message : "Không tải được chi tiết sản phẩm.");
      })
      .finally(() => {
      ...
      <main className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <Card>
          <CardContent className="p-8">
            <p className="font-bold text-red-700">{error || "Không tìm thấy sản phẩm."}</p>
            <Button asChild className="mt-5">
              <Link to="/san-pham">Quay lại sản phẩm</Link>
            </Button>
          </CardContent>
        </Card>
      </main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <Card className="bg-white">
            <CardContent className="grid min-h-[520px] place-items-center p-8">
              {product.imageUrl ? (
                <img alt={product.name} className="max-h-[520px] w-full rounded-3xl object-cover" src={product.imageUrl} />
              ) : (
                <ProductMockup />
              )}
            </CardContent>
          ...
          <div>
            ...
            <p className="mt-5 text-lg leading-8 text-stone-600">
              {product.description || "Sản phẩm cà phê phù hợp cho khách mua lẻ, quán cà phê và đơn hàng dùng thử trước khi đặt sỉ."}
            </p>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <span className="text-3xl font-black text-[var(--roast)]">{formatVnd(product.price)}</span>
              <Button disabled={!product.price} onClick={() => addItem(product)}>
                <ShoppingCart size={18} />
                {isInCart ? "Thêm tiếp vào giỏ" : "Thêm giỏ hàng"}
              </Button>
              <Button asChild variant="outline">
                <Link to="/bao-gia">Yêu cầu giá sỉ</Link>
              </Button>
            </div>
          </div>
        </div>
        ...
        <section className="mt-16">
          <div className="flex items-end justify-between gap-4">
            <div>
              <Badge>Gợi ý mẫu</Badge>
              <h2 className="mt-3 font-serif text-4xl font-black text-stone-950">Có thể phù hợp với bạn</h2>
            </div>
            <Button asChild variant="outline">
              ...
              return (
                <Card key={suggested?.id ?? title} className="bg-white">
                  <CardContent className="p-5">
                    {suggested?.imageUrl ? (
                      <img alt={title} className="h-44 w-full rounded-2xl object-cover" src={suggested.imageUrl} />
                    ) : (
                      <ProductMockup tone={["from-[#5a2f21] to-[#1f130e]", "from-[#36553a] to-[#172318]", "from-[#c47a3d] to-[#5b2f1f]"][index % 3]} />
                    )}
                    <h3 className="mt-5 text-xl font-black text-stone-950">{title}</h3>
                    <p className="mt-2 text-sm leading-6 text-stone-600">
                      {suggested?.description || "Gợi ý mẫu, sau này có thể thay bằng thuật toán đề xuất theo lịch sử mua hàng."}
                    </p>
                    {suggested ? (
                      <Button asChild className="mt-5 w-full" variant="outline">
                        <Link to={`/san-pham/${suggested.id}`}>Xem chi tiết</Link>
                      </Button>
                    ) : null}
                  </CardContent>
                </Card>
